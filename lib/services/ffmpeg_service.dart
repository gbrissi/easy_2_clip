...
import '../utils/create_folder_if_not_exists.dart';
...
class FFmpegService {
  ...
  static getFFmpegVideoFrames(
    ...
    final double secFramerate = await _getVideoFramerate(
      videoPath,
    );

    final int framesSkip = _getVideoSkip(secFramerate, step.inSeconds);

    for (var frame = startFrame;
        frame < lastFrame;
        frame = frame + framesSkip) {
  ...
  static int _getVideoSkip(double framerate, int step) =>
      (framerate * step).toInt();

  static Future<double> _getVideoFramerate(String path) async {
    final Map<dynamic, dynamic>? fileInfo = await getVidInformation(path);
    final String streamFramerate = fileInfo!["streams"][0]["avg_frame_rate"];

    final List<String> divFramerate = streamFramerate.split('/');
    final double framerate = double.parse(divFramerate[0]) /
        double.parse(
          divFramerate[1],
        );
